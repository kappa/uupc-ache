...
/*--------------------------------------------------------------------*/
 ...
 /*
  *      $Id: hostable.c 1.16 1994/05/08 21:43:33 ahd Exp $
  *
  *      $Log: hostable.c $
...
/*--------------------------------------------------------------------*/
   ...
   {
/* Ache ==============================================================*/
      sprintf( hostname, "*%s", period);
      if ((hostz = searchname(hostname, MAXADDR)) != BADHOST)
	 return hostz;
/* Ache =========================================================== */
      /* prevent ping-pong with default mailserver */
      if (equali(hostname + 2, E_domain))
	 return BADHOST;
/* ================================================================ */
...
/* Ache ============================================================== */
   mkfilename(s_hostable, E_confdir, SYSTEMS);

   ff = FOPEN(s_hostable, "r",TEXT_MODE);
   if (ff == NULL)
   {
      printerr("loadhost", s_hostable);
      panic();
   }
