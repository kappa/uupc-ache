...
/*
 *    $Id: lock.c 1.14 1994/02/19 04:43:20 ahd Exp $
 *
 *    Revision history:
 *    $Log: lock.c $
...
/*--------------------------------------------------------------------*/
   ...
   mkfilename(fname, E_spooldir, "locks.lck\\");
   strncat(fname, system, HOSTLEN);
   strcat(fname, ".");
...
/* ache ============================================================= */
   ...
   if (locket != NULL && share != NULL) {
#ifdef __OS2__
      if (locking(fileno(locket), LK_NBLCK, 0x7fffffffL) < 0) {
#else
      if (locking(fileno(locket), LK_NBLCK, -1L) < 0) {
#endif
	  if (errno == EINVAL) {
	     printmsg(0, "LockSystem: share locking not installed");
	     free(share);
	     share = NULL;
	  } else {
	     fclose(locket);
...
/*--------------------------------------------------------------------*/
   ...
   locked = TRUE;
   printmsg(6, "LockSystem: %s locked, filename %s", system, lname);
   return TRUE;
...
/*--------------------------------------------------------------------*/
   ...
   unlink( lname );
   printmsg(6, "UnlockSystem: %s unlocked", lname);
