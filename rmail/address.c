...
/*
 *    $Id: address.c 1.15 1994/05/08 21:43:33 ahd Exp $
 *
 *    Revision history:
 *    $Log: address.c $
...
/*--------------------------------------------------------------------*/
   ...
   if ( strpbrk(address,"@!") == NULL )  /* Any at signs?               */
   {                                /* No --> Look further for %      */
...
/*--------------------------------------------------------------------*/
   ...
   while ((tptr != NULL) && (*tptr == '@'))     /* Explicit RFC 822 path? */
   {
...
/*--------------------------------------------------------------------*/
   ...
   if ((hostp->status.hstatus == phantom || hostp->status.hstatus == gatewayed)
	&& hostp->realname == NULL)
      return input;
...
/* Ache ==============================================================*/
   if (strpbrk(input,"%!@"))    /* Any host delimiters?           */
	return best;
...
/* Ache ==============================================================*/
   if (hostp == BADHOST) {
      int dlen = strlen(E_domain);
      int hlen = strlen(input);

      /* prevent ping-pong with default mailserver */
      if (hlen > dlen && input[hlen - dlen - 1] == '.' &&
	  equali(E_domain, input + hlen - dlen))
	  return "";
      return best;
   }
