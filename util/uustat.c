...
/*
 *    $Id: uustat.c 1.22 1994/04/27 00:02:15 ahd Exp $
 *
 *    $Log: uustat.c $
...
/* Ache ============================================================== */
static boolean hit = FALSE, kill_all = FALSE;
...
/*--------------------------------------------------------------------*/
   ...
   while ((c = getopt(argc, argv, "amqKk:r:s:u:x:P:")) !=  EOF)
      switch(c) {
      case 'a':
	 command = LIST_ALL;
	 break;
...
/*--------------------------------------------------------------------*/
      ...
      while( readnext(fname , hostp->hostname, "C", &ltime, &size) )
      {
...
/*--------------------------------------------------------------------*/
   ...
   while  (hostp !=  BADHOST )
   {
	 ...
	 if ( access( msname , 0 ))   /* Does the host file exist?     */
	 {
	    ...
	    printmsg(-1,"Created dummy job %s (%s) for system %s",
...
/*--------------------------------------------------------------------*/
   ...
   while(hostp != BADHOST )
   {
      printmsg(-1,"%-10.10s  %s  %s", hostp->hostname,
            dater( hostp->status.lconnect , NULL ),
            hostp->status.hstatus < last_status ?
                  host_status[ hostp->status.hstatus ] :
                  "*** INVALID/UNDOCUMENTED STATUS ***");
...
static void kill_job(const char *jobid)
{
   ...
   if ( unlink( host ) )
   {
      ...
      printerr(host, "unlink");
   }
   else {
      printmsg(-1,"Deleted file %s (%s)", canon, host);
      printmsg(-1,"Killed job %s (%s) queued for host %s by %s",
...
static void refresh_job(const char *jobid)
{
   ...
   touch( host );
   printmsg(-1,"Rejuvenated job %s (%s) queued for host %s by %s",
            jobid, host, system, user);
...
/*--------------------------------------------------------------------*/
		  ...
		  case JOB_KILL:
			   ...
			   printmsg(-1,"Deleted file %s (%s)", dname, host);
                     }
                     break;
...
/*--------------------------------------------------------------------*/
   ...
   while ( current != NULL )
   {
      ...
      if ( current->execute)
	 printmsg(-1,"%-12s %s %c %-8.8s %-8.8s %s",
		     job,
	 ...
	 printmsg(-1,"%-12s %s %c %-8.8s %-8.8s %ld %s",
		     job,
                     dater( current->created, NULL ),
                     current->type,
                     sys,
...
static void usage( void )
{
   fputs("Usage:\tuustat\t[-m|-q|-a] [-K] [-u userid|all] [-s system|all] [-x debug]\n\
\t\t[-k|-r] jobid [-x debug]\n\
