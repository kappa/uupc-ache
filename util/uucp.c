...
/*
 *    $Id: uucp.c 1.15 1994/02/19 05:14:52 ahd Exp $
 *
 *    Revision history:
 *    $Log: uucp.c $
...
static char  idfile[15];       /* Unix style name for data file copy  */
static char  *uucp_name;
static char  flags[16];
static char  tempcname[128];
...
static void usage(void)
{
   fprintf(stderr, "Usage: uucp\t[-c|-C] [-d|-f] [-gGRADE] [-j]"
		   "[-m] [-nUSER] [-r] [-sFILE]\\\n"
		   "\t\t[-x|X DEBUG_LEVEL] source-files destination-file\n");
} /* usage */
             ...
             char *dest_file)
{
      ...
      for (cname=dest_file; *cname; cname++)
	if (*cname=='\\') *cname='/';

      if (*tempcname)
        cname = strrchr(tempcname, '\\') + 1;
      else
        cname = nil(char);
      namein = mktempname(NULL,"xtm");

/*--------------------------------------------------------------------*/
/*                 First - lets get the basic command                 */
/*--------------------------------------------------------------------*/

      mkfilename(xcmd, calldir, "uux.exe");
      sprintf(xcmd+strlen(xcmd), " -l -%c%d -g%c -%c%s%s%s%s%s%s%s%s%s%s <%s",
			  logecho ? 'X' : 'x',
			  debuglevel, grade,
			  spool_flag ? 'C' : 'c',
			  /* start at once at the end */" -r",
			  j_flag ? " -j" : "",
			  *spool_file ? " -s" : "",
      ...
      fin = FOPEN(namein, "w", BINARY);
      if (fin == nil(FILE)) {
        printerr("uucp", "do_uux");
        free(namein);
        return (2);
      }
      fprintf(fin,"\n%s!uucp\n-C%s%s%s",
...
/*--------------------------------------------------------------------*/
      ...
      if ((*src_syst == '\0') || equal(src_syst, nodename)) {
	 if (*src_file)
               fprintf(fin, "\n!%s", src_file);
	 else
               fprintf(fin, "\n%s", uucp_name);
...
/*--------------------------------------------------------------------*/
      ...
      if (*dest_inter != '\0')  {
	 if (*dest_syst != '\0')
          fprintf(fin, "\n(%s!%s!%s)", dest_syst, dest_inter, dest_file);
	 else
          fprintf(fin, "\n(%s!%s)", dest_inter, dest_file);
      }
      else  {
	 if ((*dest_syst == '\0') || equal(dest_syst, nodename))
          fprintf(fin, "\n(%s!%s)", nodename, dest_file);
      }
              ...
              char *dest_file)
{
      ...
      {
	 if (equal(dest_syst, nodename)) {
	    if (expand_path(dest_file, NULL, pubdir, NULL) == NULL)
	       exit(1);
	 ...
	 fprintf(cfile, "R %s %s %s -%s %s 0777 %s\n", src_file, dest_file,
	       mailbox, flags, *spool_file ? spool_file : "dummy", remote_user);
	 fclose(cfile);
         return(1);
      }
	 ...
	 if (expand_path(src_file, NULL, pubdir, NULL) == NULL)
	    exit(1);
	 normalize( src_file );
	 ...
	 while (*p)
	 {
	    ...
	    while (wildmatch(dp->d_name, search_file) != 0);
	 } /* else */
         ...
         while (write_flag)
	 {
	    ...
	    if (spool_flag)
	    {
	       sprintf(idfile, spool_fmt, 'D', nodename, (char) subseq++,
			   sequence_s);
		  ...
		  if ( lastPath != NULL )
		  {
	       ...
	       if (!cp(src_file, FALSE, idfilename, FALSE))
               {
		  printmsg(0, "do_copy: copy \"%s\" to \"%s\" failed",
			      src_file,
			      idfilename);
                  if (dirp != NULL )
                     closedir( dirp );
	    ...
	    fprintf(cfile, "S %s %s %s -%s %s 0644 %s\n", src_file,
		     *dest_file ? dest_file : idfile,
		     mailbox, flags, idfile, remote_user);
            fclose(cfile);
            if (wild_flag)  {
               do {
                   dp = readdir(dirp);
	 ...
	 (void) cp(src_file, FALSE, dest_file, FALSE);
         return(1);
      }
...
/*--------------------------------------------------------------------*/
      ...
      while ((option = getopt(argc, argv, "Ccdfg:jmn:rs:x:X:")) != EOF)  {
	 switch(option)  {
	    case 'c':               /* don't spool                    */
               spool_flag = FALSE;
               break;
      ...
      if (remote_user[0] == '\0')
      {
         ...
         strncpy(remote_user, mailbox, sizeof(remote_user) - 1 );
...
/*--------------------------------------------------------------------*/
	 ...
	 strcpy(dest_system, nodename);
      if (!dest_file[0]) {
         getcwd(dest_file, sizeof(dest_file));
	 strlwr(dest_file);
...
/*--------------------------------------------------------------------*/
	 ...
	 if ((*src_system != '\0') && (strcspn(src_file, "*?[") < strlen(src_file)))  {
	    if (do_uux(src_system, src_system, src_file, dest_system, dest_inter, dest_file))
		exit(1);
            continue;
	 }
...
/*--------------------------------------------------------------------*/
	 ...
	 if (*dest_inter != '\0')  {
	    ...
	    if (!*src_system || equal(src_system, nodename)) {
		char *s;
		...
		printmsg(4, "source: system \"%s\", file \"%s\"", src_system,
				 src_file);
		mktempname(tempcname,"xtm");
		do_copy(src_system, src_file, dest_system, "");
		sfile = "";
		if ((s = strrchr(src_file, '/')) != NULL)
			uucp_name = s + 1;
		else
			uucp_name = src_file;
		if (!*uucp_name)
		...
		sfile = src_file;
	    if (do_uux(dest_system, src_system, sfile, "", dest_inter, dest_file))
		exit(1);
	    continue;
	 }
...
static void appendSlash( char *s )
{
   ...
   if ( last )
   {
      ...
      printmsg(0,"appendSlash: Empty string for file destination");
      panic();
   } /* else */
